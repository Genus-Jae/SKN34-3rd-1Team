...
async def run(execute, offline=False):
    ...
    prompts = {"after_low": after.SUPPORT_PROGRAM_RANKING_INSTRUCTIONS}
    requests = {}
    plan = []
    for index, (name, query, industry, purpose) in enumerate(CASES):
        ...
        requests[name] = item
        plan.append((name, "after_low", 1))
    plan.append(("FUNDS", "after_low", 2))
    assert len(plan) == 6
    if not execute:
    ...
    manifest = {"status": "running", "provenance": "mock" if offline else "live_openai",
                ...
                "agentMaxTurns": 1, "model": "gpt-5.6-luna", "reasoning": "low", "maxOutputTokens": 10000,

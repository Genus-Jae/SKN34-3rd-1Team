...
async def run(execute, offline=False):
    ...
    prompts = {"before": before.SUPPORT_PROGRAM_RANKING_INSTRUCTIONS,
               "after": after.SUPPORT_PROGRAM_RANKING_INSTRUCTIONS}
    requests = {}
    plan = []
    for index, (name, query, industry, purpose) in enumerate(CASES):
        ...
        for variant in (("before", "after") if index % 2 == 0 else ("after", "before")):
            plan.append((name, variant, 1))
    plan.extend([("FUNDS", "after", 2), ("FUNDS", "after", 3), ("CONSULTING", "after", 2)])
    assert len(plan) == 13
    if not execute:
    ...
    assert os.getenv("OPENAI_MODEL", "gpt-5.6-luna") == "gpt-5.6-luna"
    ...
    manifest = {"status": "running", "provenance": "mock" if offline else "live_openai",
                ...
                "agentMaxTurns": 1, "model": "gpt-5.6-luna", "reasoning": "none", "maxOutputTokens": 10000,
    ...
    async def on_request(request):
        ...
        if (payload.get("model") != "gpt-5.6-luna" or payload.get("store") is not False
                or payload.get("max_output_tokens") != 10000 or payload.get("reasoning", {}).get("effort") != "none"):
    ...
    async def on_response(response):
        ...
        usage.append({"event": "response", "identity": active["identity"], "httpStatus": response.status_code,
                      "responseStatus": body.get("status"), "usage": safe_counts})
        write(output / "usage.json", usage)
    ...
    code = 2
    try:
        agent = SupportProgramRecommendationAgent(model=OpenAIResponsesModel(model="gpt-5.6-luna", openai_client=client),

...
class CnTradeNoticeSupportProgramCatalogSyncScheduler(private val syncService: CnTradeNoticeSupportProgramCatalogSyncService) {
    ...
    fun synchronize() {
        try {
            ...
            logger.error("충청남도 온라인수출지원시스템 공고 수집에 실패했습니다. 실패 유형: {}. 기존 공개 공고는 유지합니다.", exception.failure)
        } catch (exception: RuntimeException) {
            logger.error("충청남도 온라인수출지원시스템 공고 동기화에 실패했습니다. 오류 유형: {}. 기존 공개 공고는 유지합니다.", exception.javaClass.simpleName)
        }
    }

...
/**
 * Controller 메서드의 [Account] 파라미터를 세션 쿠키로 채웁니다.
 *
 * 파라미터가 non-null이면 세션이 없거나 만료됐을 때 [AccountSessionService.requireAccount]의 예외가 그대로 401이
 * 됩니다. nullable(`Account?`)이면 쿠키가 없는 요청에는 null을 넣어 비로그인 조회를 허용하되, 쿠키가 있는데
 * 유효하지 않으면 여전히 401입니다.
 */
class AuthenticatedAccountArgumentResolver(
    ...
    override fun resolveArgument(
        ...
        val sessionToken = SessionCookieHelper.read(request)

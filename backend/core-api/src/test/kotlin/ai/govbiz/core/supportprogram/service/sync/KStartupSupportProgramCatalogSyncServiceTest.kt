...
class KStartupSupportProgramCatalogSyncServiceTest {
    ...
    @Test
    fun publishesOnlyTheKStartupSnapshotAfterTheWholeCollectionAndIndexHaveSucceeded() {
        val programs = listOf(program("179197"), program("179198"))
        doReturn(7L).`when`(repository).startSyncGeneration("KSTARTUP")
        doReturn(programs).`when`(facade).load()
        doReturn(true).`when`(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 7L)

        assertEquals(2, service().sync())

        inOrder(repository, facade, index).apply {
            verify(repository).startSyncGeneration("KSTARTUP")
            verify(facade).load()
            verify(index).indexSnapshot(programs)
            verify(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 7L)
    ...
    fun collectionFailureRecordsOnlyItsSourceAndDoesNotIndexOrPublish() {
        ...
        doReturn(8L).`when`(repository).startSyncGeneration("KSTARTUP")
        doThrow(failure).`when`(facade).load()

        assertSame(failure, assertThrows(IllegalStateException::class.java) { service().sync() })

        verify(repository).startSyncGeneration("KSTARTUP")
        verify(repository).recordSyncFailureIfCurrent("KSTARTUP", 8L)
    ...
    fun indexFailureLeavesThePublishedSnapshotUntouched() {
        ...
        doReturn(9L).`when`(repository).startSyncGeneration("KSTARTUP")
        doReturn(programs).`when`(facade).load()
        ...
        verify(repository).startSyncGeneration("KSTARTUP")
        verify(repository).recordSyncFailureIfCurrent("KSTARTUP", 9L)
        verifyNoMoreInteractions(repository)
    }
    ...
    fun supersededSyncDoesNotRecordFailureOrPublishAnOlderGeneration() {
        val programs = listOf(program("179197"))
        doReturn(10L).`when`(repository).startSyncGeneration("KSTARTUP")
        doReturn(programs).`when`(facade).load()
        doReturn(false).`when`(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 10L)

        assertNull(service().sync())

        verify(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 10L)
        verify(repository, never()).recordSyncFailureIfCurrent("KSTARTUP", 10L)
    }

    @Test
    fun aVerifiedCompleteEmptyScopeCanPublishAnEmptySnapshot() {
        val programs = emptyList<CatalogSupportProgram>()
        doReturn(11L).`when`(repository).startSyncGeneration("KSTARTUP")
        doReturn(programs).`when`(facade).load()
        doReturn(true).`when`(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 11L)

        assertEquals(0, service().sync())
        verify(index).indexSnapshot(programs)
        verify(repository).publishSnapshotIfCurrent("KSTARTUP", programs, 11L)
    }
    ...
    fun preservesTheOriginalFailureIfFailureRecordingAlsoFails() {
        ...
        doReturn(12L).`when`(repository).startSyncGeneration("KSTARTUP")
        doThrow(failure).`when`(facade).load()
        doThrow(recordingFailure).`when`(repository).recordSyncFailureIfCurrent("KSTARTUP", 12L)
    ...
    private fun program(id: String) = CatalogSupportProgram(
        program = SupportProgram(
            id = id, sourceCode = "KSTARTUP", title = "$id 창업 지원", organization = "기관", summary = "지원 내용",
            ...
            status = SupportProgramStatus.UNKNOWN, sourceName = "K-Startup",
            sourceUrl = "https://www.k-startup.go.kr/detail?pbancSn=$id", matchedReasons = emptyList(),
        ), sortTimestamp = "",
    )
}

...
- 지역·분야는 펼쳐진 단일 선택 목록으로 표시합니다. 선택 항목을 녹색으로 강조하며 키보드 방향키로 이동할 수 있습니다.
  처음 선택지를 불러오는 중이거나 조회에 실패하면 안내를 표시하고, 실패 시 **다시 불러오기**로 재시도합니다.
...
홈 제목은 “우리 회사에 맞는 지원사업, AI와 함께 찾아보세요.”이며 지역·업종·필요한 지원을
입력하면 관련 공고와 확인할 신청 조건을 안내한다는 설명을 표시합니다. 기존 ‘AI 맞춤 검색’ 배지는 제거했습니다.
제목 두 줄은 700ms 동안 순서대로 나타나고 두 번째 줄의 녹색 강조는 3초간 한 번 재생합니다.
새 의존성이나 JavaScript 타이머 없이 CSS로 구현했고, `prefers-reduced-motion`에서는 정적으로
표시하며 강제 색상 모드에서도 글자가 보이도록 유지합니다.
...
Node 24.20.0·pnpm 11.22.0에서 전체 **42파일 518개 테스트**, lint·build를 통과했습니다.
테스트 프로세스에서는 `env -u VITE_CORE_API_BASE_URL -u VITE_DEV_PROXY_TARGET pnpm test --maxWorkers=2`로
개발 프록시 설정을 제외했습니다. 실제 Chrome에서 일반·동작 줄이기·강제 색상 7건과 로고 이동 4건을
API 대역으로 확인했으며 실제 AI 호출은 없습니다. 기존 500KB 초과 JS chunk 빌드 경고는 유지됩니다.

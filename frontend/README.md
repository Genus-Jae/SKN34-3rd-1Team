...
| (모든 화면) 오른쪽 아래 도우미 | 런처 | GovBiz 도우미(`presentation/shared/assistant`). 인사 뒤 도움말 주제 목록(지원사업 검색·중복 검토·파트너·기타)을 어느 화면에서나 같게 보여 주고 주제 → 질문 → 답으로 타고 들어가며, 관심 공고 마감·받은 제안 상태를 카드로 답하고 화면으로 이동. C1은 AI 호출 없이 동작하며 자유 질문은 추천 질문으로 안내. 로그인·회원가입·소셜 완료·예제 화면에서는 숨김 |
...
202 접수 뒤에는 3초 간격으로 이전 GET 완료 후 상태를 다시 조회합니다. 새 작성 화면의 최근 20개 분석 작업에서 재로그인·새로고침 후에도
기존 작업을 선택할 수 있습니다. 조회 실패 후 재시도는 GET만, POST 응답 유실 후 같은 화면의 재시도는 같은 요청 키만 사용합니다.

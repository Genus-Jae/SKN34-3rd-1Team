...
| (모든 화면) 오른쪽 아래 도우미 | 런처 | GovBiz 도우미(`presentation/shared/assistant`, 채널 ID 읽기는 `data/config/kakaoChannel`). 인사 뒤 도움말 주제 목록(지원사업 검색·중복 검토·파트너·기타)을 어느 화면에서나 같게 보여 주고 주제 → 질문 → 답으로 타고 들어가며, 관심 공고 마감·받은 제안 상태를 카드로 답하고 화면으로 이동. `VITE_KAKAO_CHANNEL_ID`가 있으면 빠른 답변 끝에 "담당자에게 문의"를 두고 카카오톡 채널 1:1 채팅을 새 탭으로 연다. 머리의 `⋯` 메뉴는 새 대화 하나. C1은 AI 호출 없이 동작하며 자유 질문은 추천 질문으로 안내. 로그인·회원가입·소셜 완료·예제 화면에서는 숨김 |
...
관심 공고함은 `/api/v1/me/saved-programs`를 쓰며 공고 상세의 관심 공고 저장 버튼(비로그인은 로그인 링크)이 담습니다. 진행 관리는 기존 신청 준비 목록 API를 재사용하며, 각 건의 독립된 진행 revision으로 단계 변경을 저장합니다. 로그인 작업 사이드바는 흰색 배경과 둥근 메뉴 항목을 사용하며,
...
202 접수 뒤에는 3초 간격으로 이전 GET 완료 후 상태를 다시 조회합니다. 새 작성 화면의 최근 20개 분석 작업은
공고명·제공처·요청 시각·상태가 구분된 카드로 표시하며, 재로그인·새로고침 후에도 기존 작업을 선택할 수 있습니다.
...
선택 요약은 고정 높이 한 줄에 유지해 선택 전후의 화면 밀림을 막는다. 관심 공고는 기본 화면에서 바로 펼치지 않고 `관심 공고함에서 선택` 버튼을 누를 때만 팝업에서 조회·선택하며, 전체 검색과 같은 선택 상태를 공유한다. 선택한 결과는 같은 자리에서 강조·해제하고 별도 중복 목록을 만들지 않는다. 두 번째 단계의 버튼 한 번으로 입력 저장과 분석을 연속 실행하며 단계 이동마다 스크롤을 상단으로 초기화한다. 입력 충돌 시 편집 내용을 유지하고 최신 입력 조회·명시적 적용을 제공한다.

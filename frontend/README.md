...
않으며 사용자가 검색을 제출해야 합니다. 검색 중에는 조건 편집·적용·해제·접수 상태 변경을 차단합니다.
각 검색 메시지에 당시 조건을 복사해 표시하므로 조건 변경 후에도 지난 결과를 새 조건의 결과로 오해하지 않게 합니다.

화면 검색은 `POST /api/v1/support-programs/search`에
...
검색 요청에는 현재 입력한 검색어와 사용자가 직접 적용한 기업 조건을 전달합니다. 이전 대화에서 조건을
자동 추출·갱신하는 다중 턴 대화(C02), 로그인, 북마크, 알림, 대화 이력의 서버 저장은 아직 구현하지 않았습니다.
...
| Redux 메모리 | 채팅 메시지·검색별 조건 스냅샷·입력·적용 기업 조건·편집 초안·접수 상태, Redux SampleItem | 앱 내 이동 시 유지, 새로고침 시 초기화 |
...
시작은 현재 메시지·기업 조건·편집 초안·접수 상태를 초기화하며 이전 대화 목록을 보관하지 않습니다.

...
관심 공고함(`/app/saved-programs`)은 기존 사이드바 안의 **UI 시안**입니다. 전용 ViewModel의 예시 공고로
월별 마감 달력·목록(5개씩)·지역/분야/검색·마감 공고 제외·상시/날짜 미확인 구분·상세 패널만 확인합니다.
예시 날짜는 화면 진입일(서울 기준)을 바탕으로 생성하며 실제 즐겨찾기·API·DB·AI·비교는 연결하지 않습니다.
이 단계에서는 형식적인 UseCase/Repository를 추가하지 않고 실제 서버 연결 단계에서 도입합니다.
새 검색은 채팅 화면이 맡으므로 사이드바에 따로 두지 않습니다.

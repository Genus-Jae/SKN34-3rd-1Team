...
공개 검색(`/`)은 대화 전에는 공용 헤더를, 첫 메시지 전송 후에는 검색 전용 사이드바와 계정 헤더를 사용합니다.
...
| `/app/application-preparations/new` | 사이드바 | 기업마당 공고를 카탈로그에서 검색·선택하고 공식 PDF/HWPX의 신청 문서와 문항을 찾은 뒤 신청 준비 시작. URL·ID 직접 입력은 보조 경로 |
...
초안 입력·예시 선택만으로는 화면을 바꾸거나 자동 전송하지 않습니다. 첫 메시지를 전송하면 왼쪽 탐색 메뉴,
오른쪽 위 로그인·무료 회원가입, 중앙 대화 영역과 하단 입력창으로 전환합니다.
`GuestSearchLayout`이 대화 상태에 맞춰 헤더·탐색 메뉴를 표시하며 새 채팅, AI/필터 검색 방식,
파트너 모집, 요금제와 계정 진입점을 담당합니다. 사이드바 아래에는 비회원 2건·로그인 후 최대 5건 공개 정책을 안내합니다.
데스크톱에서는 사이드바를 접고 펼칠 수 있고, 760px 미만에서는 메뉴 버튼으로 네이티브 모달을 열어
포커스 이동·Esc 닫기를 지원합니다. 검색 방식은 대화 전에는 가로 탭(좌·우 화살표),
대화 후에는 세로 탭(위·아래 화살표)이며 Home/End도 지원합니다.
사용자 메시지는 오른쪽 회색 말풍선에, 답변과 결과는 여백 있는 본문에 표시합니다.
취소·오류 뒤에는 대화를 유지하고 사이드바의 새 채팅은 기존 소개 화면으로 돌아옵니다.
비로그인 채팅 입력창 아래에는 중복되는 새 검색 버튼을 표시하지 않습니다.
새 채팅은 필터 검색에서도 AI 탭으로 돌아와 대화·조건·초안을 비우고 입력에 포커스를 돌려줍니다.
...
READY 수신 후에는 이전 추가 질문 문맥을 종료합니다. 입력을 바꾸면 카드를 숨기되 제안 조건은 보관하여
다음 발화의 해석에 전달합니다. 제안 취소나 새 대화는 제안과 미확정 초안을 모두 무효화합니다.
...
공개 헤더·검색 사이드바의 `GovBiz 홈으로` 링크는 `/`를 새 문서로 열어 같은 홈 주소에서도 대화·초안·필터를

...
- 필터 검색은 공고명·기관명 키워드, 지역·분야, 출처(전체·기업마당·K-Startup), 접수 상태를 선택하고 **검색**으로 적용합니다.
...
- K-Startup을 선택하면 접을 수 있는 **추가 조건**에서 창업 업력·신청 대상·대표자 연령을 고릅니다.
  ...
  출처를 전체·기업마당으로 바꾸면 전용 조건을 비웁니다. 추가 조건 역시 공고 분류이며 신청 자격 판정이 아닙니다.
...
K-Startup 공고도 Core 카탈로그 API를 통해 필터 검색·상세 조회합니다. 원문 근거 질문은 계속 미지원입니다.

...
공개 검색(`/`)은 민트색 배경, 짙은 녹색의 둥근 상단바, 중앙 소개 문구와 넓은 검색 입력창으로 구성합니다.
첫 화면의 별도 환영 말풍선은 소개 영역으로 대신하며 Redux 대화 상태는 유지합니다. 예시 질문은 입력만
채우고 자동 전송하지 않습니다. 참고 디자인에 있는 기관·공고 수, 무료 검색 횟수·잔여 한도·요금제는
실제 구현된 정책이 아니므로 표시하지 않습니다. 브랜드는 GovBiz를 유지합니다.
모든 화면은 `src/index.css`의 민트·딥그린 색상 변수를 공유합니다. 로그인·회원가입, 파트너 모집,
기업 프로필, 관리자, 공고 상세·원문 질문, 상태관리 예제에도 흰색 둥근 카드·녹색 버튼·입력창을
적용합니다. 공개 화면의 상단바와 작업 화면의 사이드바는 딥그린으로 통일하고, 오류·주의 상태는
구별할 수 있도록 적색·황색을 유지합니다.
작업 채팅(`/chat`)은 같은 녹색 입력창·결과 스타일을 사용하되 사이드바와 하단 입력 배치를 유지합니다.
새 검색은 초안 입력만으로 나타나지 않으며, 대화가 시작되면 입력 박스 아래쪽 안내 옆에 표시합니다.
...
공개·모바일 채팅은 새 제안·응답이 생기면 해당 내용을 문서 스크롤로 보여 줍니다. 데스크톱 채팅은 내부
타임라인을 스크롤하고, 상세에서 복귀할 때 마지막 내용을 표시합니다. 초안 편집만으로 문서를 이동하지 않습니다.
...
**새 검색** 버튼은 대화 또는 확정 검색이 있을 때 나타나며 대화·검색 의도·기업 조건·접수 필터·미확정 제안을
...
`READY`는 검색 의도를 제목으로, 실제 변경할 기업 조건·접수 상태만 작은 칩으로 표시하고
**이 조건으로 검색** 또는 **제안 취소**를 제공합니다. 값이 지워지는 조건은 `현재 소재지 해제`처럼
명시합니다. 미입력·변경 없는 항목과 현재/제안 비교표는 표시하지 않으며, 유지되는 유효 조건이 있으면
`나머지 조건은 유지됩니다.`로 요약합니다. 추가 확인이 필요할 때는 질문과 짧은 답변 안내만 표시합니다.
READY 수신 후에는 이전 추가 질문 문맥을 종료합니다. 입력을 바꾸면 READY 제안을 폐기하고, 제안 취소나 새 대화는
제안과 미확정 초안을 모두 무효화합니다. 해석은 공고 준비 상태와 독립적으로 허용하고 실제 확인 검색만

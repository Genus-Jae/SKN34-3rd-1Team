...
공개 검색(`/`)은 흰색 배경과 둥근 상단바, 중앙 소개 문구와 넓은 검색 입력창으로 구성합니다.
채팅 입력창의 연녹색 테두리·둥근 모서리·그림자·포커스 효과는 유지합니다.
첫 메시지를 전송하면 소개와 예시 질문을 숨기고 대화 영역 위·입력창 아래의 배치로 전환합니다.
초안 입력이나 예시 선택만으로는 전환하지 않으며, 취소·오류 뒤에도 대화 배치를 유지합니다.
새 검색을 누르면 중앙 소개 배치로 돌아오고, 상세 공고에서 복귀하면 기존 대화 배치를 복원합니다.

...
describe('지원사업 직접 필터 검색', () => {
  it('출처 선택은 요청 없이 추가 조건을 펼치고 응답 전 기본값과 응답 후 초안을 보존한다', async () => {
    ...
    const inputs = fields.map(({ label, values, value }) => {
      const input = screen.getByRole('combobox', { name: label }) as HTMLSelectElement
      expect([...input.options].map((option) => option.value)).toEqual(['', ...values])
    ...
    fields.forEach(({ label, values, value, extra }, index) => {
      ...
      expect([...input.options].map((option) => option.value)).toEqual(['', ...values, extra])
    })
    ...
    expect((screen.getByRole('combobox', { name: '창업 업력' }) as HTMLSelectElement).value).toBe('3년미만')
  })
  ...
  it.each(['', 'BIZINFO'])('출처를 %j로 바꾸면 추가 조건을 비우고 검색 시 URL과 요청에서 제외한다', async (sourceCode) => {
    ...
    await screen.findByRole('link', { name: startupProgram.title })
    fireEvent.change(screen.getByRole('combobox', { name: '출처' }), { target: { value: sourceCode } })
    expect(screen.queryByRole('button', { name: /K-Startup 추가 조건/ })).toBeNull()
  ...
  it('추가 조건 검색은 첫 페이지로 이동하고 이전 조회를 취소하며 초기화로 전부 지운다', async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledOnce())
    fireEvent.click(screen.getByRole('button', { name: /K-Startup 추가 조건/ }))
    fireEvent.change(screen.getByRole('combobox', { name: '창업 업력' }), { target: { value: '예비창업자' } })
    fireEvent.click(screen.getByRole('button', { name: '검색' }))
    await screen.findByRole('link', { name: startupProgram.title })
  ...
  it('뒤로 가기에서 출처·추가 조건과 목록에 없는 선택값을 복원한다', async () => {
    ...
    await screen.findByRole('link', { name: startupProgram.title })
    expect((screen.getByRole('combobox', { name: '창업 업력' }) as HTMLSelectElement).value).toBe(original.startupStage)
    ...
    expect((screen.getByRole('combobox', { name: '창업 업력' }) as HTMLSelectElement).value).toBe(original.startupStage)
    expect((screen.getByRole('combobox', { name: '신청 대상' }) as HTMLSelectElement).value).toBe(original.applicantType)
    await screen.findByRole('link', { name: startupProgram.title })
  })
  ...
  it('K-Startup 오류 재시도는 적용 조건을 유지하고 미적용 추가 조건을 덮어쓰지 않는다', async () => {
    ...
    expect((screen.getByRole('combobox', { name: '창업 업력' }) as HTMLSelectElement).value).toBe('예비창업자')
  })
  ...
  it.each([
    ...
    expect((screen.getByRole('combobox', { name: '출처' }) as HTMLSelectElement).value).toBe('KSTARTUP')
    expect(fetchMock).toHaveBeenCalledOnce()
    fireEvent.click(screen.getByRole('button', { name: '다시 불러오기' }))
    await screen.findByRole('link', { name: startupProgram.title })
    expect(fetchMock).toHaveBeenCalledTimes(2)
  ...
  it.each(['/', '/app/chat'])('%s의 K-Startup 상세 복귀에 추가 조건을 보존하고 미지원 질문을 열지 않는다', async (path) => {
    ...
    await screen.findByRole('link', { name: startupProgram.title })
    expect((screen.getByRole('combobox', { name: '출처' }) as HTMLSelectElement).value).toBe('KSTARTUP')
    expect((screen.getByRole('combobox', { name: '창업 업력' }) as HTMLSelectElement).value).toBe(startupParams.startupStage)

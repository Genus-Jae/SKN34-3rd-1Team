...
const helpTipStyles = {
  ...
  tooltip:
    'absolute left-0 top-full z-20 mt-2 flex w-[19rem] max-w-[min(19rem,80vw)] flex-col gap-2 rounded-[0.85rem] border border-sample-border bg-white p-3 text-left text-[0.78rem] font-normal leading-[1.55] text-app-ink shadow-[0_12px_32px_rgb(32_33_36_/_12%)]',
  title: 'm-0 text-[0.72rem] font-extrabold text-sample-muted',
} as const

/**
 * `?` 버튼에 붙는 도움말 말풍선입니다. 마우스를 올리거나 키보드 포커스가 오면 열리고 Esc로 닫히며, 말풍선 위로
 * 마우스를 옮겨도 유지됩니다. 버튼 기준 절대 위치라 뒤 화면의 배치는 밀지 않습니다.
...
export function HelpTip({ label, title, children }: { label: string; title?: string; children: ReactNode }) {
  ...
  return (
    <span className={helpTipStyles.root} onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)}>
      ...
      {isOpen ? (
        <div id={tooltipId} className={helpTipStyles.tooltip} role="tooltip">

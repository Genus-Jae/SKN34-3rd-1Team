...
const repository = { list: vi.fn(), get: vi.fn(), create: vi.fn(), replace: vi.fn(), runs: vi.fn(), run: vi.fn(), start: vi.fn(), source: vi.fn() }
beforeEach(() => {
  ...
  appContainer.register({ combinationReviewUseCase: asValue(new CombinationReviewUseCase(repository)) })
})
afterEach(() => { cleanup(); vi.unstubAllGlobals(); appContainer.register({ combinationReviewUseCase: asValue(original), browseSupportProgramsUseCase: asValue(originalCatalog) }) })
...
describe('review screens and execution safety', () => {
  ...
  it('updates the acknowledged input version on empty PUT success without automatic GET or analysis', async () => {
    ...
    mount(); await screen.findByDisplayValue(reviewFixture.title)
    fireEvent.change(screen.getByLabelText('검토 제목'), { target: { value: '수정된 제목' } })
    fireEvent.click(screen.getByText('입력 저장'))
  ...
  it('retains one logical request after response loss and across remount, with no automatic POST', async () => {
    ...
    const view = mount(); await screen.findByDisplayValue(reviewFixture.title)
  ...
  it('shows auth expiry and clears personal view', async () => {
    ...
    expect(screen.queryByText('검토 입력과 실행 이력')).toBeNull()
  })
  ...
  it('uses the run snapshot order, six stages and source index', async () => {
    ...
    expect(screen.getAllByText(/PDF 3쪽, 문단 2/)).toHaveLength(6)
    expect(repository.start).not.toHaveBeenCalled()
  })

...
describe('PartnerRecruitmentUseCases', () => {
  it('browse trims the keyword and region and falls back to the first page', async () => {
    const browse = vi.fn().mockResolvedValue({ recruitments: [], total: 0, page: 1, pageSize: 20, totalPages: 0 })
    await new BrowsePartnerRecruitmentsUseCase({ browse }).execute({ keyword: ' 스마트 ', seekingRole: '', region: ' 서울 ', mineOnly: false, sort: 'DEADLINE', page: 0 })
    expect(browse).toHaveBeenCalledWith({ keyword: '스마트', seekingRole: '', region: '서울', mineOnly: false, sort: 'DEADLINE', page: 1 }, undefined)
  })

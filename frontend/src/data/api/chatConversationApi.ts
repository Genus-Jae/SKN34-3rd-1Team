...
export async function chatConversationRequest<T>(accountEmail: string, path: string, schema: z.ZodType<T>, signal?: AbortSignal, body?: unknown): Promise<T> {
  ...
  try {
    const response = await fetch(`${getCoreApiBaseUrl()}/api/v1/me/chat-conversations${path}`, {
      method: body === undefined ? 'GET' : 'PUT', credentials: 'include', cache: 'no-store', signal: controller.signal,

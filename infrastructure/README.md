...
검증 스크립트는 `verification` profile의 `bizinfo-stub`·`kstartup-stub`·`openai-stub`을 사용합니다. MySQL·Qdrant는
실제 서버이고, 외부 공고·임베딩·점수화 응답만 고정된 가상 자료입니다. 기업마당 공고 27개와 K-Startup 공고 2개를
수집하고, 최신 20개 밖의 기업마당 AI 공고와 K-Startup 공고가 함께 검색되는지, 전용 3개 필터가 저장된 분류를
사용하는지 확인합니다. 이는 서비스 연결과 후보 누락 수정의 검증이지,
실제 OpenAI 모델의 검색 품질 측정이 아닙니다.

스텁 주소와 더미 인증키를 강제하므로 개인 키를 외부로 전송하거나 실제 OpenAI 비용을 발생시키지 않습니다.
기업마당·K-Startup 스텁은 디코딩된 키도 확인합니다. 두 제공처 동기화와 색인은 `PT2S` 주기로 실행합니다.

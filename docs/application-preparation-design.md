...
## 10. 챕터별 PR 경계와 브랜치 운영

작업 브랜치는 `skn-89` 하나를 유지한다. 챕터 하나를 구현·검증해 PR로 병합한 뒤 팀 `main`의 병합 결과를 같은
브랜치에 반영하고 다음 챕터를 시작한다. GitHub의 자동 삭제로 원격 브랜치가 없어져도 로컬 `skn-89`에서 같은 이름으로
다시 올린다. 각 PR에는 해당 챕터 이후의 코드를 미리 섞지 않는다.

```text
skn-89: 챕터 1 커밋 → PR 1 병합
  → 최신 upstream/main 반영
  → 챕터 2 커밋 → PR 2 병합
  → 최신 upstream/main 반영
  → 다음 챕터 반복
...
병합 뒤에는 먼저 `upstream/main`을 fetch하고 브랜치에 병합 결과를 반영한다. 이미 공유한 브랜치 이력을 임의로
재작성하지 않으며, 새 PR의 변경 파일과 커밋 범위에 이전 챕터가 다시 포함되지 않는지 확인한다.

| 챕터 | PR 결과물 | 필수 검증 |
|---|---|---|
| 1. 제약·계약 | 이 문서, 공식 대상·범위·API/AI 계약 | 링크·원문 위치·`git diff --check` |
| 2. 양식 manifest·Domain | 검수 manifest, Domain 규칙·테스트 | Core 전체 테스트 |
| 3. DB·Repository | 새 Flyway migration, MyBatis, Repository·통합 테스트 | MySQL 8.4 Testcontainers 포함 Core 전체 테스트 |
| 4. AI Service | typed 계약, prompt, 단일 Agent, Service·Router·테스트 | AI Service 전체 pytest |
| 5. Core 공개 API | 인증·소유권·AI Client·실행·오류 계약 | Core 전체 및 AI Stub 계약 테스트 |
| 6. Frontend 데이터 연결 | Entity·UseCase·Repository·API/Zod·DI | Frontend test·lint·build |
| 7. Frontend 작업 화면 | 경로·목록·문항·질문·초안·확인·세션 격리 | Frontend test·lint·build |
| 8. 서비스 통합 | Compose Stub 흐름, 문서·운영 한계 정리 | 변경 서비스 전체·계약·Compose·`git diff --check` |

각 챕터에서는 production 코드·관련 설정·테스트 코드를 모두 작성한 뒤 검증을 마지막 한 단계로 모아 실행한다.
이전 PR의 통과 결과를 다음 챕터 변경의 검증으로 대신하지 않는다. Stub·자동 테스트 통과를 실제 신청 문서 품질이나
기관 검수 완료로 표현하지 않는다. 실제 OpenAI 품질 평가는 전송 자료와 호출 예산을 별도로 승인받은 뒤 실행한다.
...
- 후속 챕터가 하나의 수직 범위를 넘지 않도록 PR 경계가 정해진다.

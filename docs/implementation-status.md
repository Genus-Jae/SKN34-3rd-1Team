...
| 신청 문서 작성 도우미 질문·사실 확인 | 구현됨·초안 작성 미구현 | 기본 생성·목록·상세에 `POST .../sections/{sectionKey}/messages`, `PUT .../inputs`를 연결. AI는 이번 사용자 답변의 정확한 인용으로 사실·미정 항목만 제안하며 자동 저장하지 않음. 사용자가 값과 근거를 확인한 전체 스냅샷만 MySQL에 저장하고 전역 입력 revision을 증가시킴. 요청 키 재실행, 소유권, revision 충돌, AI 실행 이력을 구분함. 초안 생성·직접 수정·특정 버전 확인은 후속 기능. [설계와 경계](application-preparation-design.md) |

...
| 신청 문서 작성 도우미 공고 검색·양식 발견·질문·사실 확인 | 구현됨·초안 작성 미구현 | 선택 공고를 검색 결과 위에 올리고 같은 카드에서 신청 문서 찾기를 실행. 원문 인용의 공백·줄바꿈 차이와 반복 AI 식별자를 안전하게 정규화하고 AI 출력 위반과 실제 출처 오류를 구분. 저장 목록은 확인 후 본인 준비 건만 삭제하며 하위 사실·AI 실행 기록은 cascade 삭제하고 공용 양식 스냅샷은 유지. 다른 제공처·HWP·스캔 PDF/OCR·파일 업로드와 초안 생성·수정·확인은 후속 기능. [설계와 경계](application-preparation-design.md) |

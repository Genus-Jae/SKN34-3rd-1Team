...
이번 전체 검증은 Frontend **287개**, AI **459개**, JDK 21·실제 MySQL 8.4 Core **508개**가 통과했고
...
| 대화 맥락 기반 조건 변경 | 구현됨 | 작은 검색 의도·확정 조건·마지막 확인 질문과 초안만으로 변경 제안. 유지/변경/해제 비교 후 확인 검색, 상대 업력은 정확한 날짜를 질문. 전체 이력이나 영구 저장은 없음 |
| 조건 해석 취소·재시도 | 구현됨 | 해석 40초·검색 90초 제한, 늦은 응답 무시. 제안 확인 전 폼 수정·취소·새 대화 시 초안 폐기. 확인 검색 재시도에서는 해석을 반복하지 않음 |

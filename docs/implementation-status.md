...
| 신청 문서 작성 도우미 공고 검색·양식 발견·질문·사실 확인 | 구현됨·초안 작성 미구현 | 작성 도우미 안에서 공고명·기관명으로 기업마당 공고를 검색·선택하며 URL·ID 직접 입력은 보조 경로로 제공. 선택만으로 분석하지 않고 사용자가 명시적으로 요청하면 공식 PDF/HWPX를 수집해 정확한 원문 인용이 있는 신청 문서·문항만 AI가 제안. 파일 hash·파서·모델·프롬프트 버전이 고정된 MySQL 스냅샷을 재사용하고 발견 양식을 확인한 뒤 준비 건을 생성. 다른 제공처·HWP·스캔 PDF/OCR·파일 업로드와 초안 생성·수정·확인은 후속 기능. [설계와 경계](application-preparation-design.md) |

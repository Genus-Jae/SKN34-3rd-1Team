...
후속 C01은 **익명 기업 조건 입력·적용과 실제 검색 반영**으로 범위를 좁혔습니다. 지역·업종·설립일·지원 목적을
명시적으로 적용하고, 검색마다 당시 조건을 표시합니다. [C01 구현·검증 기록](company-conditions-search.md)을 참고하세요.
이번 C01 검증은 Frontend **208개**, AI Service **256개**, JDK 21·실제 MySQL 8.4 Core **421개** 테스트가
...
기업 단계 구분, 자연어 후속 조건 갱신(C02), 첨부까지 포함한
전체 신청 요건 대조(E01 전체), 영구 프로필 저장은 아직 구현하지 않았습니다.
...
K-Startup 공식 URL 표시와 RAG 미지원 사전 안내를 검증했습니다. 실제 K-Startup Client·공고 수집·추가 데이터의
가치 표본 비교와 로그인·북마크는 미구현입니다. 이번 준비의 유료 API 호출은 0회이며,
...
| 검색 준비 상태 확인 | 구현됨 | 필수 `sources`에 제공처별 상태·공고 수·색인 준비·동기화 시각 표시. 일부만 준비된 `SEARCHABLE_WITH_PARTIAL_SOURCES`도 검색 허용 |
| 접수 상태 선택 | 구현됨 | 채팅 UI에서 접수 중/전체를 선택해 `acceptingOnly`에 반영. 기본은 `true`, 전체는 예정·마감·상태 미확인도 포함 |
| 익명 기업 조건 검색 | 구현됨 | 현재 소재지·업종·설립일·지원 목적을 적용·수정·개별 해제·전체 초기화. POST body로 후보 검색과 AI 점수화에 반영 |
| 검색 당시 조건 표시 | 구현됨 | 각 검색의 조건을 메모리에 보관해 이후 변경된 조건과 구분. 입력 중인 초안은 명시적 적용 전까지 검색에 반영하지 않음 |
| 최신 목록 조회 | API 구현됨 | `query=`로 요청하면 AI 없이 최신 공고 최대 5개 반환; 채팅 UI는 빈 입력을 전송하지 않음 |
| 추천 결과 카드 | 구현됨 | 제목·기관·신청 기간·출처·관련도 점수와 대상·지역 자격 검토·본문 인용. 조건 확인/확인 필요 묶음 구분, 첨부 미검증 명시 |
...
| 대화 맥락 기반 검색 | 미구현 | 직접 적용한 기업 조건은 다음 검색에도 유지하지만 이전 메시지에서 조건을 추출·갱신하지 않음 |
| 회원·영구 기업 프로필·북마크·알림 | 미구현 | 조건은 현재 대화 메모리에서만 유지. 저장·인증 API와 해당 업무 흐름 없음 |
...
| 두 번째 공고 제공처 | 준비 기능 구현, 실제 연동 미구현 | K-Startup 공식 URL·표시 이름, 제공처별 준비/검색/복구 경계 완료. 실제 수집 Client·동기화·데이터 가치 비교 없음 |
...
제공처가 섞이면 전체 응답을 거부합니다. 실제 두 번째 제공처를 추가할 때는 수집·정규화·동기화와 데이터의
추가 가치를 검증해야 하며, K-Startup 원문 질문은 여전히 미지원입니다.
...
| 대상·지역 불일치 제외 | 구현됨 | 공식 API 본문 우선. `INCOMPATIBLE`은 제외, `UNKNOWN`은 확인 필요 묶음으로 분리; 둘 다 `MATCH`인 공고 우선 |
| AI 응답 검증 | 구현됨 | 후보 ID·중복·점수 범위·합계·묶음별 정렬·자격 필드·추천 이유·동일 후보 본문의 정확한 인용·절단 시 UNKNOWN 검증 |
...
| AI Service | Agent 출력·점수·자격 필터·정확한 본문 인용·일반·원문 근거 벡터 API·Compose 스텁 계약 테스트. 원문 우선 수정 후 전체 319개 통과 | 내부 계약과 검색·근거 청크 색인 동작 확인. 실제 모델 정확도·실제 Qdrant 연동 검증과는 구분 |
...
| 우선순위 | 작업 | 완료를 판단할 기준 |
|---|---|---|
| 1 | K-Startup 가치 확인 후 실제 연동 | API 제공 후 응답·페이지 완전성과 공고 표본으로 기업마당 대비 추가 가치를 먼저 확인하고, 채택 후 Client·정규화·제공처별 동기화 구현 |
...
| 4 | 사용자 기능 확장 | 로그인·영구 기업 프로필·북마크의 제품 범위를 정한 뒤 API·저장·화면까지 연결. C01의 익명 조건 입력과는 별도 범위 |

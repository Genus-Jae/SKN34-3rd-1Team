...
| 기업 등록·기본정보 | 구현됨 | `GET/POST/PUT /api/v1/me/company`, `GET .../lookup`. 사업자등록번호를 Bizno 조회로 확인해 계속사업자만 등록하고 상호·사업자 상태를 저장. 소재지·업종·설립연도·홈페이지는 담당자 입력. 등록하면 `tier=COMPANY`. `/app/profile`의 기본정보 카드가 등록·수정 폼과 완성도를 실제 값으로 표시 |
| 이메일 인증, 비밀번호 변경·계정 삭제 | 미구현 | 다음 단계. 가입 계정은 `emailVerified=false`. 인증이 생기면 `COMPANY` 조건에 더하고 파트너 제안은 인증만으로 허용 |
| 협업·파트너 설정, 우대·인증 자격, 알림, 북마크 | 미구현 | 프로필 화면에 준비 중으로 표시 |

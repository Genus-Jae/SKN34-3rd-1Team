...
| 신청 문서 작성 도우미 공고 기반 양식 발견·질문·사실 확인 | 구현됨·초안 작성 미구현 | 기업마당 공고 ID·URL에서 공식 PDF/HWPX를 수집하고 정확한 원문 인용이 있는 신청 문서·문항만 AI가 제안. 파일 hash·파서·모델·프롬프트 버전이 고정된 MySQL 스냅샷을 재사용하고 사용자가 발견 양식을 확인한 뒤 준비 건을 생성. 이후 답변의 사실·미정 제안과 사용자 확인 입력 저장·revision 흐름을 연결. 다른 제공처·HWP·스캔 PDF/OCR·파일 업로드와 초안 생성·수정·확인은 후속 기능. [설계와 경계](application-preparation-design.md) |

...
가치 표본 비교와 북마크는 미구현입니다(로그인은 이후 구현). 이번 준비의 유료 API 호출은 0회이며,
...
| 회원가입·이메일 인증·기업 등록 | 미구현 | 다음 단계. 계정 스키마(`role`, `email_verified_at`, `suspended_at`, `deleted_at`)는 준비됨. `COMPANY` 단계는 기업 등록과 함께 추가 |
...
| 스키마 관리 | 구현됨 | Flyway V1 공고, V2 동기화 세대, V3 공고별 공식 원문, V4 공개 스냅샷 준비 상태, V5 계정·세션 테이블 |
...
현재 Compose는 개발 환경이며 회원가입, 운영 접근 제어, 배포 자동화, 백업·복구 절차는 별도 구현·검증이

...
| 신청 문서 작성 도우미 공고 검색·양식 발견·질문·사실 확인 | 구현됨·초안 작성 미구현 | 전체 제공처 공고를 검색하고 기업마당·과기정통부 공고를 선택해 공식 PDF/HWPX를 분석. 공고 선택과 발견 문서 확인을 두 단계로 분리하고 완료 시 결과 제목으로 포커스를 이동. 원문 인용의 공백·줄바꿈 차이와 반복 AI 식별자를 안전하게 정규화하고 AI 출력 위반과 실제 출처 오류를 구분. 저장 목록은 확인 후 본인 준비 건만 삭제하며 하위 사실·AI 실행 기록은 cascade 삭제하고 공용 양식 스냅샷은 유지. K-Startup·충남 수출지원 첨부, HWP·스캔 PDF/OCR·파일 업로드와 초안 생성·수정·확인은 후속 기능. [설계와 경계](application-preparation-design.md) |

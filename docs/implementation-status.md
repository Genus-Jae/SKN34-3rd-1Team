...
| 신청 문서 작성 도우미 기본 흐름 | 구현됨·AI 작성 미구현 | `GET /api/v1/application-preparations/forms`, `POST/GET /api/v1/application-preparations`와 상세 조회. 공식 HWPX hash·문항 위치를 고정 manifest로 관리하고, 로그인 계정이 지원 분야를 선택해 준비 건을 만든 뒤 목록·상세에서 다시 엶. `/app/application-preparations`·`/new`·상세와 사이드바 연결. 문항 입력·질문·초안·수정·확인은 후속 기능이며 조회만으로 AI를 호출하지 않음. [설계와 경계](application-preparation-design.md) |
...
| 파트너 모집글 작성·목록·상세 | 구현됨 | `GET/POST /api/v1/partners/recruitments`, `GET .../{id}`. 기업을 등록한 회원이 제공처에 현재 있는 접수 중 공고 하나에 모집글을 쓰고(공고당 하나, 마감일은 접수 마감 전날까지), 누구나 검색어·찾는 역할(복수)·지역(복수, 고른 지역과 전국 모집글이 함께 나옴)·정렬·페이지로 읽음. 프런트 목록은 검색어·역할·지역을 조회 버튼으로 적용하고 지역 선택지에서 전국을 빼 "전체"가 전국까지 뜻함. 모집 상태는 마감일·공고 접수 마감·수동 마감으로 조회 시점에 계산. 프런트 목록·상세·작성 화면이 이 API를 쓰며 매칭·추천은 예시. 계약은 [계정·인증 계약](account-auth-contract.md#파트너-모집글). 비로그인 공개 화면(`/partners`, 상세)은 작성 기업 정보(상호·소재지·업종·인증 여부)를 흐린 자리표시자로 가리고, 자세히 보기·제안 버튼이 "로그인하면 할 수 있는 일" 다이얼로그(배경 흐림, 로그인·가입 뒤 같은 모집글로 복귀)를 엶. 로그인 뒤 화면은 사이드바 "파트너 관리" 한 항목 아래 한 줄 머리글(제목 "파트너 관리" · 모집글/제안함 탭 · 작성 버튼)로 오가고, 목록 카드는 폭에 따라 3·2·1열에 하단 줄을 맞추며 긴 태그는 말줄임. 작성 화면의 우리 기업·작성 팁은 `?` 도움말 말풍선으로 옮기고 공고 원문 확인 카드는 제거 |

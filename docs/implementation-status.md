...
| 비밀번호 변경·계정 삭제 | 구현됨 | `PUT /api/v1/me/password`, `GET /api/v1/me/deletion-preview`, `DELETE /api/v1/me`. 변경은 로그인 세션만으로 본인을 확인해 새 비밀번호만 받고 요청한 세션만 남기며, 삭제는 현재 비밀번호를 다시 확인(불일치 422)하고 삭제는 보낸 대기 제안 철회·내 모집글 마감·기업 삭제·세션 삭제·`deleted_at`을 한 transaction으로 처리. 프로필 계정 카드의 두 모달(공용 `WorkspaceModal`)이 연결됨(삭제 모달의 정리 항목 목록은 문구 확정 전까지 숨김). 삭제 시 이메일을 익명화해 같은 이메일로 다시 가입 가능 |

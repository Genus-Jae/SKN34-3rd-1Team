...
- 관련 이슈: [#123 — skn-59](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN34-3rd-1Team/issues/123)
- 상태: **입력 API와 공식 근거 초안에 이어 자동 첨부 수집·PDF/HWPX 파싱·단일 Agent·실행 이력(4-2~4-4)을 연결했다. 실제 모델 품질·사람 검수·화면·운영 배포는 미완료다.**
- 설계 기준: 2026-09-09, 소스 커밋 `f238d993bd0a4c4f109ff109d39f09e5c0e54c8c`.
- V6는 검토 입력, V7은 실행 스냅샷·원본 파일을 정의한다. 공식 원문과 AI Service 호출은 분석 POST에서만 실행한다.
...
자동 수집은 현재 기업마당 숫자형 공고 ID와 직접 연결된 기업마당/중기부 PDF·HWPX를 지원한다.
...
| `CombinationReviewInput` | 서로 다른 사업 2~3개를 확정 입력으로 검증하고 목록 스냅샷 보존 |
| `ReviewProgramPair` | 서로 다른 두 식별자를 정규화한 비교 대상 |

`programPairs()`는 2개 선택 시 1쌍, 3개 선택 시 3쌍을 반환한다. 입력 순서를 바꿔도 같은 쌍과 비교 순서를 반환하고,
...
Domain 테스트는 ID 길이·특수문자·중복, 제공처·세부사업 분리, 3개 사업의 모든 순서, 스냅샷 보호를 확인한다.
...
V6는 기존 migration을 수정하지 않고 새 테이블만 추가한다. 계정을 물리적으로 삭제하면 해당 검토·자식 행은 FK cascade로 정리된다.
기존 계정의 논리 삭제는 행을 물리 삭제하지 않으며 위 인증 검사로 접근을 제한해야 한다.
3-1에서는 현재 입력만 저장했다. 현재는 V7 Run에 당시 입력·원문·분석 결과를 별도로 보존한다.
...
- `programs`는 2~3개다. 동일 제공처·공고·세부사업 조합은 중복으로 거절하고 표시 순서를 유지한다.
...
- 독립적인 참여 사실·UNKNOWN 기본값, 2~3개 사업·중복·ID·제목·Unicode 경계·revision 타입·null 입력 검증.
...
| client/mapper | PDF/HWPX 디코딩·첨부 DTO→문서 모델, AI 요청/응답 DTO 변환 |
...
1. 사용자 URL 대신 `BIZINFO:PBLN_<숫자>` 식별자로 공식 상세 페이지를 구성한다. null이 아닌 subProgramId와 다른 제공처는 미지원이다.
2. 공고 상세 영역의 직접 첨부를 추출한다. 연결된 중기부 사업공고 페이지가 있으면 그 페이지의 첨부도 확인한다.
3. HTTPS·정확한 허용 호스트·경로·파일 쿼리만 사용한다. 첨부 다운로드 redirect는 거절하며 다른 호스트로 자동 추적하지 않는다.
4. PDF/HWPX를 읽는다. 같은 표제의 중기부 HWPX가 있으면 기업마당 변환본보다 우선하고 선택 사유를 기록한다.
5. 미지원 부속 자료는 coverageWarnings에 기록한다. 지원 문서가 하나도 없거나 읽는 파일에 실패하면 정상 판단 대신 기술 오류다.

한 파일 최대 16 MiB, 공고당 최대 4개·32 MiB, PDF 80쪽, HWPX 256 ZIP 항목·압축 해제 합계 24 MiB,
문서당 60,000 UTF-16 길이·256블록, 전체 120,000 길이·512블록을 제한한다. 초과 내용을 잘라내지 않는다.
HWP·ZIP 내부 첨부 탐색·스캔 PDF OCR·암호화 문서는 미지원이다. 빈/이미지만 있는 PDF 페이지도 필요한 예외를 놓칠 수 있어 거절한다.
...
현재 AI 시간 초과도 ANALYSIS_UNAVAILABLE로 저장된다. 정상적인 근거 부족과 구분하며 504 세분화는 아직 하지 않았다.
...
- 제공처/공고 ID로 중복 선택을 막고 2~3개 사업을 저장한다. UNKNOWN과 독립적인 수행 상태를 유지한다.
- expectedRevision으로 전체 입력을 수정하고 409에서도 편집을 보존한다. 최신 저장 입력 조회와 편집 폐기는 별도 버튼이다.
- 명시적인 분석 버튼만 POST한다. 응답 유실 시 탭 sessionStorage에 보관한 같은 키·버전·추가 설명으로 수동 확인한다.
...
브라우저 검증은 `frontend/scripts/combination-review-mock.mjs`의 가상 응답만 사용했다. 원본 다운로드도 모의 바이트의
...
Core를 최신 코드로 재빌드·재시작하면 Flyway가 V6/V7을 반영한다. 분석에는 최신 AI Service도 필요하다.
